...
BOT_TOKEN = environ.get('BOT_TOKEN', "8261229851:AAGGgQFVWMTZKo4IdWogeP7mnd1YYYuIJ3M")
...
# Admins & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5974804314').split()] # For Multiple Id Use One Space Between Each.
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '-1003410930811').split()]  # For Multiple Id Use One Space Between Each.
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1003634285978')).split()]  # For Multiple Id Use One Space Between Each.
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Surya1234:Surya1234@sbmoviebot.ugrufcs.mongodb.net/?appName=sbmoviebot")   # IF Multiple Database Is False Then Fill Only This Database Url.
DATABASE_NAME = environ.get('DATABASE_NAME', "sbmoivebot")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'sbcollection')
...
PAYMENT_QR = environ.get('PAYMENT_QR', 'https://graph.org/Payment-QR-CODE-12-28') # payment code picture url.
